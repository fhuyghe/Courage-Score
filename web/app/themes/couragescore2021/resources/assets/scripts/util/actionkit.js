...
forms.isUQDDonor = forms.isUQDStepComplete = forms.isUQDJourneyComplete = forms.isUQDDonationComplete = forms.isUQDRedirectInitiated = forms.isUQDEnrollmentPromptComplete = false;
forms.initUQD = function(iframe, options){
    ...
    var fallbackRedirectDelay = 5 * 1000;
	...
	// don't repeatedly initiate a redirect
	if ( !forms.isUQDRedirectInitiated ) {
	    forms.isUQDRedirectInitiated = true;
	    window.location.assign(actionkit.context.quickdonate_redirect);
	}
    }
    var sendToAfterActionOnceAfter = function(delay){
	setTimeout(sendToAfterActionOnce, delay);
    }
    var isUQDComplete = function(){
	var isComplete = forms.isUQDDonationComplete && forms.isUQDStepComplete && forms.isUQDJourneyComplete;
	return ((forms.isUQDDonor || forms.isUQDEnrollmentPromptComplete) && isComplete)
    }
    var connection = Penpal.connectToChild({
		...
		$log('Donation complete.');
		forms.isUQDDonationComplete = true;
		// UQD donors are immediately redirected to TY page,
		// other donors given the chance to enroll
		if (isUQDComplete())
		    sendToAfterActionOnceAfter(fallbackRedirectDelay);
            },
            journeyComplete: function(data) {
		$log('Journey is complete.');
		forms.isUQDJourneyComplete = true;
		if (isUQDComplete())
		    sendToAfterActionOnceAfter(fallbackRedirectDelay);
            },
            stepComplete: function(data) {
		$log('Step is complete.');
		forms.isUQDStepComplete = true;
		if (isUQDComplete())
		    sendToAfterActionOnceAfter(fallbackRedirectDelay);
            },
	    selectedEnrollmentOption: function(data){
		// triggered for unenrolled donors after they've donated
		// and uncookied UQD donors doing a oneclick donation
		if(forms.isUQDDonor){
		    // Donor remains logged in even if they don't complete the oneclick donation
		    $log('Uncookied UQD donor didn\'t complete oneclick donation.');
		    return;
		}
		forms.isUQDEnrollmentPromptComplete = true;
		if (isUQDComplete())
		    sendToAfterActionOnceAfter(fallbackRedirectDelay);
	    },
	    formBeforeUnload: function(data){
		if ( !isUQDComplete() ){
		    $log('Form unloaded but action not complete: logged out, navigated away, etc.')
		    return;
		}
		$log('QuickDonate form in process of being unloaded, redirect to TY.');
		sendToAfterActionOnce();
	    },
	    redirectPageHit: function(url){
		$log('Hit AK landing page, redirect to TY.');
		// don't check for completion here -- it's as complete as it's going to be
		sendToAfterActionOnce();
	    },
...
forms.insertUQDIframe = function(formOptions, messagingOptions){
    var url;
    // ok for this not to be parameterized -- height is adjusted after the UQD form loads
    var css = '<style>#uqd-frame {width:100%; height:665px; border:0px;}</style>';
    var baseURL = 'https://' + formOptions.hosts.quickdonate + '/';
    if (formOptions.token) {
        url = baseURL + 't/quickdonate/' + formOptions.quickdonatePage + '/' + formOptions.token;
    } else {
        var qsObj = {'amount': formOptions.amounts.join('x')}
        // Prefill amount if it's one of the options.
        // Don't prefill other amount, UQD form doesn't recognize it's been selected
        // and validation fails: "Please select an amount."`
        // But that's only true for iframe, not when hitting UQD form directly. Weird.
        if (ak.args.prefill && ak.args.amount && formOptions.amounts.indexOf(ak.args.amount) > -1){
            qsObj['default_amt'] = ak.args.amount;
    ...
    try {
        ...
        messagingOptions['host'] = 'https://' + formOptions.hosts.fallback;
        forms.initUQD(iframe, messagingOptions);
    } catch (e) {
