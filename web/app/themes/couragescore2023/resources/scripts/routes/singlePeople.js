...
domReady(async () => {
  ...
  if ($('#mapContainer').length > 0) {
    ...
    var map = new mapboxgl.Map({
      ...
      zoom: 4,
      maxBounds: [
        [-130.580095, 30.3674814],
        [-110.9696226, 44.0922698],
      ],
    });
    ...
    map.on('load', function () {
      ...
      let districtUrl =
        $('.body').html() == 'assembly'
          ? 'https://services3.arcgis.com/fdvHcZVgB2QSRNkL/arcgis/rest/services/Legislative/FeatureServer/0/query?outFields=*&where=OBJECTID=' +
            districtNumber +
            '&f=geojson'
          : districtNumber % 2 == 0
          ? 'https://services3.arcgis.com/fdvHcZVgB2QSRNkL/arcgis/rest/services/Legislative/FeatureServer/1/query?outFields=*&where=OBJECTID=' +
            districtNumber +
            '&f=geojson'
          : 'https://services5.arcgis.com/0CGHmi5SBMvfD65u/arcgis/rest/services/AD_SD_2011_2021_SDRulesAssign/FeatureServer/5/query?where=SD_2011=' +
            districtNumber +
            '&f=geojson';
  ...
  voteTable.column(0).search('floor_votes', true, false).draw();
      ...
      if (val) {
        ...
        score = res.data
          ...
          .reduce((acc, row, idx) => {
            ...
            return Math.round((acc + parseInt(row.score)) / (idx + 1));
          }, 0);
